...
public class EmailService {
    ...
    public void sendPasswordResetEmail(String toEmail, String resetToken) {
        ...
        try {
            ...
            sendGridEmailService.sendEmail(toEmail, "StudentHub - Password Reset Request", htmlContent);
    ...
    public void sendHtmlEmailWithAttachments(String toEmail, String userName, String subject, String messageContent,
        ...
        try {
            ...
            // Note: SendGrid attachments require additional implementation
            // For now, sending without attachments
            if (attachments != null && attachments.length > 0) {
                System.out.println("⚠️ Attachments not yet supported with SendGrid integration");
            }
    ...
    private String createBulkEmailTemplate(String userName, String subject, String messageContent) {
        return "<!DOCTYPE html>" +
                ...
                "                            <h2 style='color: #2d3748; margin: 0 0 8px; font-size: 22px; font-weight: 600;'>" + subject + "</h2>" +
                ...
                "                                <p style='color: #1a202c; margin: 0 0 12px; font-size: 15px; font-weight: 600;'>Dear " + userName + ",</p>" +
                "                                <div style='color: #2d3748; margin: 0; font-size: 14px; line-height: 1.8;'>" + formatMessageContent(messageContent) + "</div>" +
                "                            </div>" +
                "                            " +
                "                            <!-- Signature -->" +
    ...
    private String createPasswordResetEmailTemplate(String resetUrl) {
        return "<!DOCTYPE html>" +
                ...
                "                                <a href='" + resetUrl + "' style='display: inline-block; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: #000000; padding: 14px 40px; text-decoration: none; border-radius: 10px; font-weight: 600; font-size: 15px; box-shadow: 0 6px 20px rgba(102, 126, 234, 0.4);'>Reset Password</a>" +
                "                            </div>" +
